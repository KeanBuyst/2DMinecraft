...
inline uint8_t Util::PerlinNoise::hash(const int32_t i) {
    return perm[static_cast<uint8_t>(i)];
}

float Util::PerlinNoise::grad(int32_t hash, float x) {
...
float Util::PerlinNoise::grad(int32_t hash, float x, float y) {
...
void Util::InitNoise()
{
    ...
    // Init Cave noise
    for (uint8_t& i : cave_noise.perm) {
        i = dist(rng);
    }
}

// range [-1,1]
float Util::PerlinNoise::noise(float x) {
...
// range [-1,1]
float Util::PerlinNoise::noise(float x, float y) {
